...
public class Principal extends JFrame {
	...
	private void createToolBars() {
		...
		firstNTButton.addActionListener(event -> {
			...
			Gramatica gramatica = GramaticaParser.textToGramatica(conteudoComponente);
			Map<Simbolo, VEstrela> firstTodaGramatica = GramaticaUtils.calcularFirst(gramatica);
			Map<Simbolo, List<Simbolo>> followTodaGramatica = GramaticaUtils.calcularFirstNT(gramatica, firstTodaGramatica);
		...
		parsingButton.addActionListener(event -> {
			...
			if (GramaticaUtils.isGramaticaLL1(gramatica)) {
				...
				JOptionPane.showMessageDialog(null, "A gramática não é LL(1). Logo, a tabela gerada pode ser inconsistente!");
			}
		...
		this.createLayout(toolbar1, toolbar2, panelGramatica);
	}
	...
	private void createLayout(JComponent... arg) {
		...
		gl.setHorizontalGroup(
				gl.createParallelGroup().addComponent(arg[0], GroupLayout.DEFAULT_SIZE, GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE).addComponent(arg[1]).addComponent(arg[2]));
